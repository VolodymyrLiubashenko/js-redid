...
burger.addEventListener('click',function(){
	...
	nav.classList.toggle('is-hieden')
...
let elementsNumberSolveSlider
let elementsNumberReviews
...
function checkScreenWidth (){
	if(window.innerWidth < 1024){
		...
		elementsNumberSolveSlider = 9
		elementsNumberReviews = 5
	}
}
...
function createLogoSlide (el) {
	...
	for(i = 0; i < elementsNumberReviews; i++ ){
		...
		if(elementsNumberReviews == 3){
			li.dataset.left = dataLeftArrayMobile[i]
		}
		else{
			li.dataset.left = dataLeftArray[i]
		}
...
function setBarPosition(that){
	...
	slideBar.style.left = that.dataset.left
		}
	}
}
...
function setBarPositionautomaticly(){
	for(el of logoItem){
		if(reviewsCarusel.children[1].dataset.logo == el.dataset.logo){
			slideBar.style.left = el.dataset.left
		}
	}
}
